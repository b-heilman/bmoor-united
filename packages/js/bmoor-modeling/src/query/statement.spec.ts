...
describe('@bmoor-modeling::query/statement', function () {
	describe('::validate', function () {
		...
		it('should not return anything if everything is fine', function () {
			...
			stmt.addModel('bar', {
				fields: [{path: 'aValue'}],
				joins: [{toSeries: 'foo', mappings: [{from: 'fooId', to: 'id'}]}],
			});
		...
		it('should catch a disconnected head', function () {
			...
			stmt.addModel('bar', {
				fields: [{path: 'anotherValue'}],
			});

			stmt.addModel('hello', {
				series: 'world',
				fields: [{path: 'world'}],
				joins: [{toSeries: 'bar', mappings: [{from: 'fooId', to: 'id'}]}],
			});

			expect(stmt.validate()).to.deep.equal(['Series foo is detached']);
		});
		...
		it('should catch a disconnected model', function () {
			...
			expect(stmt.validate()).to.deep.equal(['Series world is detached']);
		});
	});
});
