...
addEvent(event: Event, features: FeatureValues): Node {
    const intervalData = addEvent(this, event);

    intervalData.eventFeatures = new Features(features);
...
hasEvent(interval: Interval): boolean {
    return (
        this.hasIntervalData(interval) &&
        'eventFeatures' in this.getIntervalData(interval)
    );
}

getEvent(interval: Interval): Features {
    return this.getIntervalData(interval).eventFeatures;
}
