...
export function select(
	...
	if (selector.sort) {
		nodes.sort((a, b) =>
			selector.sort(
				a.getFeatures(selector.interval),
				b.getFeatures(selector.interval),
			),
		);
	}
 ...
 * data flow is event => node.weight, never allow a event to be set from node.features
 */
...
export class GraphSelection {
	...
	sort(nodeCompare: string | GraphSelectionComparator): void {
		...
		if (typeof nodeCompare === 'string') {
			...
			nodeCompare = (featuresA, featuresB) =>
				featuresA.get(field) - featuresB.get(field);
		}

		this.nodes.sort((nodeA, nodeB) =>
			(<GraphSelectionComparator>nodeCompare)(
				nodeA.getFeatures(interval),
				nodeB.getFeatures(interval),
			),
		);
		this.sorted = true;
	}
	...
	rank(nodeCompare: GraphSelectionComparator) {
		...
		const scores = this.nodes.reduce((map, node) => {
			const score = intervals.reduce((score, interval) => {
				...
				// which other events and my own event should
				// be compared
				...
				return (
					score +
					related.reduce(
						// run event compare, compute a score
						(sum, otherNode) =>
							sum +
							nodeCompare(
								node.getFeatures(interval),
								otherNode.getFeatures(interval),
							),
						0,
					)
				);
		...
		this.nodes.sort((nodeB, nodeA) => {
			...
			for (const event of events) {
				if (
					nodeCompare(
						nodeA.getFeatures(event.interval),
						nodeB.getFeatures(event.interval),
					) > 0
				) {
					sum += 1;
				} else {
	...
	eachInterval(fn: (node: Features, event: Features, prev: Features) => void) {
		...
		for (const node of nodes) {
			...
			let prev = previous ? node.getFeatures(previous) : null;
			for (const interval of intervals) {
				const cur = node.getFeatures(interval);

				if (node.hasEvent(interval)) {
					fn(cur, node.getEvent(interval), prev);
				}
	...
	transfer(from: string, to?: string) {
		...
		this.eachInterval((nodeFeatures, eventFeatures) => {
