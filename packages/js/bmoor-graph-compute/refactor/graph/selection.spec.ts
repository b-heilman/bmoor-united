...
describe('bmoor-graph::GraphSelection', function () {
	...
	describe('::transfer', function () {
		it('should properly copy from event to node weight', function () {
			const select = new GraphSelection(graph, {
			...
			expect(
				graph.getNode('p-1-1').getFeatures(graph.getInterval(1)).get('to'),
			).to.deep.equal(100);

			expect(
				graph
					.getNode('p-2-2')
					.getFeatures(graph.getInterval(1))
					.get('other'),
			).to.deep.equal(111);
		});
		...
		it('should allow sort by subSelect after transfer', function () {
			...
			const sub1 = select.subSelect({
				sort: (a: Features, b: Features) => b.get('to') - a.get('to'),
			});

			const sub2 = select.subSelect({
				sort: (a: Features, b: Features) => b.get('other') - a.get('other'),
			});
		...
		it('should allow sort after transfer', function () {
			...
			select.sort(
				(a: Features, b: Features) => b.get('other') - a.get('other'),
			);
	...
	describe('::rank', function () {
		describe('on low level nodes', function () {
			it('should allow you to compute a rank on the first interval', function () {
				...
				select.rank((featuresA, featuresB) => {
					return featuresA.get('yards') - featuresB.get('yards');
				});
		...
		describe('on top level nodes', function () {
			it('should allow you to compute a rank on the first interval', function () {
				...
				select.rank((featuresA, featuresB) => {
					return featuresA.get('score') - featuresB.get('score');
				});
